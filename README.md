...
- **特效系统**: 基于Canvas的高性能视觉特效，支持鼠标点击、拖尾、按住等多种交互特效
- **个性化定制**: 亮/暗主题、样式定制、动画效果、特效配置等
- **数据管理**: 支持数据导入导出、自动清理等功能
- **竖屏移动端适配**: 针对竖屏手机优化侧边栏、工具弹窗、草稿纸与音乐播放器，同时保持桌面端布局效果
...
- **特效系统**: Canvas 2D + requestAnimationFrame（高性能动画）
...
- **对话系统**：
  ...
  - **组件系统**：
    - 支持智能体在对话中调用可配置的组件
    - 组件调用格式：`@<!组件名~参数1,参数2,...>`
    - 内置组件：柱状图、雷达图、饼状图、进度条、表格、卡片、统计、开关、列表、配置
    - 自定义组件：用户可以创建自己的组件并让AI调用
    - 组件参数类型支持：字符串、数值、布尔、数组、对象
    - 智能参数解析，自动识别参数类型并进行转换
    - 组件帮助界面，查看所有可用组件及其使用方法
    - 组件编辑器，支持AI辅助创建组件
    - 组件导入导出功能，备份和分享自定义组件

### 图像生成
- **通用AI图像生成设置**：支持多种图像生成服务提供商
- **服务提供商选择**：
  - SD API：支持本地 Stable Diffusion WebUI
  - 网络服务商：支持硅基流动等在线图像生成服务
- **SD API 配置**：
  - 自定义 API URL（支持本地和远程地址）
  - 模型选择（自动获取可用模型列表）
  - 参数配置：采样步数、CFG Scale、尺寸、采样方法
  - 正面/负面提示词设置
- **网络服务商配置**：
  - API Key 管理
  - 模型选择：Qwen/Qwen-Image、Kwai-Kolors/Kolors
  - 尺寸配置：支持 256-2048 像素，独立控制宽度和高度
  - 步数和 CFG Scale 配置
- **基于对话内容的AI提示词生成**：自动根据对话上下文生成图像提示词
- **图像生成与展示**：实时显示生成进度，支持重新生成
- **图片显示/隐藏控制**：支持展开和隐藏生成的图片
- **图片导出功能**：支持下载生成的图片
...
- **沉浸式播放器**：
  ...
  - 非活跃歌词行模糊效果，突出当前播放歌词
  - 快-慢曲线动画，提供流畅的滚动体验
  - 竖屏模式下自动切换为上下结构：封面和歌曲信息在上，居中歌词位于下方主阅读区域
  - 竖屏歌词使用独立定位策略，当前歌词略高于视觉中心，上方已显示内容少于下方未显示内容
- **右键菜单功能**：
  - 用户歌单右键菜单：编辑歌单、删除歌单
...
### 竖屏移动端适配
- 仅对竖屏移动设备启用响应式优化，桌面端布局与视觉效果保持不变
- 主侧边栏在竖屏下切换为抽屉式交互，并补充遮罩层与快捷打开按钮
- sidebar 内的智能体列表 / 会话列表支持自适应高度，底部工具区可完整铺满容器
- 灵动岛、快速对话、草稿纸、图片生成器等工具面板针对安全区和小屏高度进行了专门优化
- 所有工具弹窗在竖屏全屏模式下移除圆角与多余边距，避免边缘留白
- 草稿纸工具补充了显式关闭按钮，便于单手操作

### 样式与主题
- **主题系统**：
  - 亮/暗主题切换
  - 自动主题切换（根据系统时间）
  - 预制主题（浅色、深色、蓝色、绿色、紫色等）
  - 自定义主题配色（支持主背景色、次背景色、文本颜色等完整配色方案）
  - AI生成主题配色（输入意象自动生成配色方案）
  - 音乐封面颜色联动（自动从音乐封面提取主题色）
- **组件颜色模式**：
  - 单色模式：使用单一主色调
  - 双色模式：使用主色调和副色调
  - 渐变模式：使用两个渐变色
  - 高级渐变模式：支持多个渐变色和自定义方向
- **字体设置**：
  - 系统字体检测与选择
  - 主字体和副字体配置
  - 字体大小选择（小、中、大）
...
- **问候动画设置**：
  - 启用问候动画开关（控制是否显示开启动画）
  - 问候动画停留时间调节（300-2000毫秒）
  - 标题文字动画类型选择（放大、飞入、渐入）
  - 风格化字体显示（Montserrat字体，细体效果）
  - 平滑的动画过渡效果
- **流光效果**：
  - 流光效果开关
  - 流光颜色自定义
...
- **特效系统**：
  ...
  - 特效颜色模式：
    ...
    - 随机颜色：使用预设的随机颜色
  - **鼠标点击特效**：
    - 8种特效类型：爆炸、涟漪、闪烁、烟花、彗星、花瓣、流星、星雨
    - 特效速度调节（0.5-5x）
    - 重力效果调节（0-0.5）
    - 粒子大小调节（1-10px）
    - 粒子数量调节（5-30个）
    - 消失速度调节（0.01-0.1）
  - **鼠标拖尾特效**：
    - 8种特效类型：粒子、连线、波浪、发光、彗星、火焰、魔法、能量
    - 特效速度调节（0.5-3x）
    - 粒子大小调节（1-8px）
    - 消失速度调节（0.01-0.1）
    - 拖尾长度调节（20-100个）
  ...
  - 完整适配主题/组件颜色系统
- **UI组件优化**：
  - 水平滑块组件：用于离散值选择（如字体大小、动画速度等）
  - 自定义滑块组件：用于连续值调节（如透明度、尺寸等）
  - 自定义下拉选择组件：用于选项选择
  - 自定义复选框组件：用于开关选项
...
- 配置 API URL（支持本地和远程地址）
- 应用将通过代理访问本地地址

**网络服务商配置**：
- 配置 API Key（如硅基流动）
- 选择模型：Qwen/Qwen-Image 或 Kwai-Kolors/Kolors
- 配置图片尺寸（256-2048 像素）
- 配置采样步数和 CFG Scale
...
└── src/                   # 源代码目录
    ...
    │   └── variables.css
    └── utils/             # 工具类
        ├── agentComponents.js     # 智能体组件系统（支持多种参数类型）
...
### v4.1.0 竖屏移动端适配优化
- 📱 **主界面竖屏适配**：
  - 主侧边栏改为抽屉式展开，避免在小屏竖向空间中挤压对话区
  - 智能体列表和会话列表支持自适应高度，sidebar 内容可完整铺满容器
  - 灵动岛在竖屏模式下做了紧凑化处理，并补充安全区适配
- 🧰 **工具弹窗全屏优化**：
  - 音乐播放器、草稿纸、快速对话、图片生成器等工具在竖屏下支持真正全屏展示
  - 移除了全屏弹窗下的圆角、边距和阴影留白问题
  - 统一补充 `safe-area-inset` 处理，改善刘海屏 / 灵动岛设备体验
- 📝 **草稿纸与工具交互修复**：
  - 草稿纸工具栏补充关闭按钮
  - 竖屏下工具操作按钮与工具栏间距重新调整，更适合单手触控
- 🎵 **音乐播放器竖屏体验增强**：
  - 沉浸播放界面改为更适合竖屏阅读的上下布局
  - 歌词改为居中对齐，并放置在封面与歌曲信息下方
  - 竖屏歌词滚动使用独立定位算法，不影响桌面端原有歌词定位逻辑

### v4.0.0 特效系统全新上线
- ✨ **Canvas 特效系统**：
  - 基于原生 Canvas 2D API 的高性能视觉特效系统
  - 全屏覆盖，支持所有页面元素（包括弹窗、播放器等）
  - 使用 requestAnimationFrame 实现流畅的 60fps 动画
  - 完整适配项目的主题/组件颜色系统
- 🎯 **鼠标点击特效**：
  - 8种精美特效：爆炸、涟漪、闪烁、烟花、彗星、花瓣、流星、星雨
  - 每种特效都有独特的视觉表现和物理效果
  - 可配置参数：速度、重力、粒子大小、粒子数量、消失速度
  - 智能颜色适配，自动匹配当前主题色
- 🌊 **鼠标拖尾特效**：
  - 8种拖尾效果：粒子、连线、波浪、发光、彗星、火焰、魔法、能量
  - 连线特效支持动态连接，形成连续的视觉路径
  - 可配置参数：速度、粒子大小、消失速度、拖尾长度、连线宽度
  - 流畅的拖尾效果，增强鼠标交互体验
- ✋ **鼠标按住特效**：
  - 6种按住效果：扩散、脉冲、旋转、花朵、漩涡、星系
  - 0.5秒延迟触发机制，避免误触发
  - 特效跟随鼠标移动，提供实时反馈
  - 可配置参数：大小、强度、速度
- 🎨 **颜色适配系统**：
  - 三种颜色模式：主题颜色、组件颜色、随机颜色
  - 完美适配单色、双色、渐变、高级渐变等所有颜色模式
  - 智能颜色回退机制，确保在任何情况下都有合适的颜色
  - 自动从主题/组件颜色系统提取颜色，保持视觉一致性
- ⚙️ **详细配置选项**：
  - 每种特效都有独立的开关控制
  - 所有参数都可实时调节，立即生效
  - 界面友好的配置界面，易于使用
...
- 🚀 **性能优化**：
  - 高效的粒子管理，自动清理过期粒子
  - 智能事件监听，按需启用特效
  - 优化的渲染算法，降低 CPU/GPU 占用
  - 平滑的动画过渡，无卡顿现象

### v3.9.0 问候动画全新上线
- 🎬 **开启动画功能**：
  - 全新的开启动画效果，黑色背景配合"UNLIMITED"标题
  - 风格化字体显示，使用 Montserrat 字体，细体效果
  - 平滑的动画过渡，提供优雅的启动体验
  - 自动隐藏功能，停留后渐隐消失
- ⚙️ **问候动画设置**：
  - 启用问候动画开关，用户可自由控制是否显示开启动画
  - 问候动画停留时间调节（300-2000毫秒），满足不同用户需求
  - 标题文字动画类型选择：
    - 放大：从下方飞入并缩放
    - 飞入：从左侧滑入
    - 渐入：简单的淡入效果
  - 所有设置自动保存到本地存储
- 🎨 **视觉优化**：
  - 使用 Google Fonts 加载 Montserrat 字体
  - 多层文字阴影效果，增强视觉冲击力
  - 字体平滑渲染，确保跨平台一致性
  - 动画曲线优化，使用 Material Design 标准缓动曲线
- 🔧 **技术优化**：
...
### v3.8.0 组件调用提示词优化
- 📝 **组件调用提示词优化**：
  - 改进组件使用说明的生成逻辑，使AI在调用组件时传入的参数更加准确
  - 更详细的格式说明，明确说明组件调用的语法格式
  - 参数分隔符说明，明确指出使用波浪号(~)分隔组件名和参数，使用逗号(,)分隔多个参数
  - 参数值限制说明，提醒AI不要在参数值中包含逗号、波浪号或特殊符号
  - 必填参数标记，清楚标记哪些参数是必填的
  - 参数类型信息，显示参数的类型（如果有）
  - 默认值信息，显示参数的默认值（如果有）
  - 更清晰的示例，提供了更完整的调用示例
- 📖 **组件帮助界面增强**：
  - 添加更详细的使用说明，包括语法格式、使用示例和注意事项
  - 语法格式说明，详细解释了组件调用的每个部分
  - 使用示例，提供了提示词和回复的完整示例
  - 注意事项，列出了使用组件时需要注意的重要事项
  - 可视化展示，使用代码块和列表使说明更加清晰
  - 添加样式支持，为新增的说明部分添加了样式
- 🔧 **IndexedDB修复**：
  - 修复每次初始化时删除数据库的问题，确保数据持久化
  - 删除了总是删除旧数据库的代码，改为正常的版本升级机制
  - 添加重建数据库功能，用户可以手动重建数据库（清除所有数据）
  - 修复组件保存和加载问题，确保自定义组件能够正确持久化

### v3.7.0 多对话模式全新上线
- 💬 **多对话模式功能**：
  - 为同一智能体创建多个独立的对话会话
  - 鼠标悬停在选中的智能体上显示多对话提示按钮
  - 点击多对话按钮进入多对话模式
  - 多对话模式下侧边栏显示对话列表，智能体列表隐藏
  - 创建智能体按钮替换为创建对话按钮
  - 支持创建、切换、删除对话会话
  - 每个对话会话独立存储，互不干扰
  - 对话会话支持导入导出功能
  ...
  - 至少保留一个对话会话，防止误删
- 🎨 **UI优化**：
  - 多对话提示按钮在智能体列表中网格布局显示
  - 2x2网格：第一行为多对话按钮，第二行为编辑和删除按钮
  - 对话列表卡片式设计，支持hover效果
  - 当前对话高亮显示
  - 退出多对话按钮，快速返回智能体列表
- 🔧 **Dynamic Island 按钮优化**：
  - 多对话模式下显示：导出对话、清理对话、导入对话
  - 普通模式下显示：导出智能体、清理聊天记录、智能体记忆、总结
- 💾 **数据存储优化**：
  - 对话会话数据使用 localStorage 存储
  - 每个会话独立存储，键名格式：`chat_session_{agentId}_{sessionId}`
  - 会话列表独立存储，键名格式：`chat_sessions_{agentId}`
  - 切换会话时自动保存当前对话
  - 发送消息、编辑、删除等操作自动保存到当前会话
- 🐛 **Bug 修复**：
  - 修复切换对话会话时数据丢失的问题
  - 修复多对话模式下保存对话到错误位置的问题
  - 优化对话保存逻辑，确保数据正确存储

### v3.6.0 图像生成系统全面升级
- 🎨 **通用AI图像生成设置**：
  - 将 Stable Diffusion 设置改为通用的 AI 图像生成设置
  - 支持多种图像生成服务提供商
- 🌐 **服务提供商选择**：
  - SD API：支持本地 Stable Diffusion WebUI
  - 网络服务商：支持硅基流动等在线图像生成服务
...
- 🐛 **Bug 修复**：
  - 修复图片隐藏后无法再次显示的问题
  - 修复图片加载时 imageExpanded 属性未初始化的问题
  - 修复 AI 辅助功能中的解析错误（content.trim 和 content.split 错误）
...
- **组件系统增强**：
  - 全新的组件参数类型系统，支持5种参数类型：字符串、数值、布尔、数组、对象
  - 智能参数解析，自动识别参数类型并进行转换
  - 支持嵌套结构（数组、对象），智能分割参数，处理引号和特殊字符
  - 新增5个示例组件展示不同参数类型：卡片、统计、开关、列表、配置
  - 为所有内置组件添加参数类型定义，实现新旧格式兼容
  - 增强渲染支持，为新组件类型添加完整的渲染逻辑和样式
  - 改进提示词生成，显示参数类型（中文名称）、默认值和更清晰的参数说明
  - 保持向后兼容，没有定义参数类型的组件继续使用旧的简单分割方式
...
- **对话会话数据**：使用 localStorage 存储，每个会话独立存储
  - 会话列表：`chat_sessions_{agentId}`
  - 会话对话：`chat_session_{agentId}_{sessionId}`
