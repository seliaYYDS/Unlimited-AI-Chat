...
### v3.5.0 样式设置组件优化
- 🎨 **UI组件优化**：
  - 创建水平滑块组件（HorizontalSlider），用于离散值选择
  - 将字体大小、圆角大小、动画速度、流光速度改为使用水平滑块
  - 将灵动岛边框宽度改为使用自定义滑块，支持 0-10px 精确调节
- 🎯 **组件选择优化**：
  - 离散值选项（small/medium/large）使用水平滑块
  - 连续值范围（如边框宽度）使用自定义滑块
  - 类别选择（如消息气泡样式）使用下拉选择
  - 开关选项使用复选框
- ✨ **水平滑块特性**：
  - 横向排列选项，节省垂直空间
  - 滑块指示器显示当前选中位置
  - 平滑动画效果（cubic-bezier 缓动函数）
  - 支持亮色和暗色主题
  - 响应式设计，移动端友好
- 🔧 **技术优化**：
  - 修复滑块位置计算问题，确保滑块准确定位到对应选项
  - 优化动画效果，从 transform 改为 left 属性过渡
  - 添加通知设置和弹窗背景设置的 CSS 变量支持
